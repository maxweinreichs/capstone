...
def _generate_prices_for_store0(base_prices_store0, variation_factor, n_productos):
    ...
    #fallback
    if base_prices_store0 is None: 
        base_prices_store0 = np.full((n_productos,), 30.0) 
    ...
    if n_tiendas > 1:
        ...
        for tienda_idx in range(1, n_tiendas):
            for prod_idx in range(n_productos):
                ...
                #muestreo uniforme no negativo
                precio_tienda_actual = np.random.uniform(
                    max(0.01, limite_inferior_arbitraje), 
                    max(0.011, limite_superior_arbitraje) 
                )

                if limite_superior_arbitraje < limite_inferior_arbitraje: 
                     precio_tienda_actual = max(0.01, precio_ref) 
                
                particle_matrix[prod_idx, tienda_idx] = precio_tienda_actual
    return np.maximum(particle_matrix, 0.01)


def generate_particle(n_productos, n_tiendas, precios_base_matrix, 
    ...
    if precios_base_matrix.ndim == 2:
        ...
        #fallback
        print(f"      Forma de precios_base_matrix inesperada ({precios_base_matrix.shape}) en generate_particle. Usando default.")
    ...
    #aplicar restriccion de no arbitraje para otras tiendas
    particle = _apply_arbitrage_constraint(particle, costo_transporte)
    return particle


def generate_particle_historical(n_productos, n_tiendas, precios_historicos_matrix, 
    ...
    if precios_historicos_matrix is None:
        ...
        precios_base_tienda0_actual = precios_historicos_matrix[:, 0] 
    elif precios_historicos_matrix.ndim == 1 and len(precios_historicos_matrix) == n_productos: 
...
def particle_filter_optimization_multi_resample(
    ...
    evaluate_fn, 
    semana_idx, 
    costo_transporte=DEFAULT_COSTO_TRANSPORTE 
):
    ...
    if precios_historicos_semana_completa is not None:
        ...
        print(f"    Fallback: Usando precios base de tienda 0 para semana {semana_idx}")
    ...
    if not initial_scores:
        print("    No se generaron partículas iniciales. Devolviendo precios base (o default) y utilidad negativa.")
    ...
    if not all_scores_acum: 
        print("    No se evaluaron partículas en total (después de rondas). Devolviendo precios base (o default) y utilidad negativa.")
        fallback_particle_tienda0 = precios_partida_tienda0_ref if precios_partida_tienda0_ref is not None else np.full((n_productos,), 30.0)
        return generate_particle(n_productos, n_tiendas, fallback_particle_tienda0, 0.0, costo_transporte), -float('inf')
